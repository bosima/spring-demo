package cn.bossma.springdemo.actuator.metric.repository;
...
@Repository
public class OrderRepository implements MeterBinder {
    @Autowired
    ...
    public Mono<Long> save(ProductOrder order) {
        var orderId = template.insert(ProductOrder.class)
                .into("t_order")
                .using(order)
                .map(o -> o.getId())
                .flatMap(id ->
        ...
        this.orderCountr.increment();
        return orderId;
    }

    @Override
    public void bindTo(MeterRegistry meterRegistry) {
        this.orderCountr = meterRegistry.counter("order.count");
    }
}
